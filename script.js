...
window.onscroll=function(){
    if(document.body.scrollTop>200||document.documentElement.scrollTop>200) backTop.style.display="block";
...
function highlightNavbar(){
    ...
    sections.forEach((id,idx)=>{
        ...
        if(rect.top<=100 && rect.bottom>=100) nav.classList.add('active');
...
function addComment(){
    ...
    newReview.classList.add("review-card","show");

    let starsHTML='';
    for(let i=1;i<=5;i++){
        starsHTML += `<span class="star ${i<=selectedRating?'filled':''}">★</span>`;
    }
    ...
    document.getElementById("total-rating").textContent=`Average Rating: ${avg} ★`;
